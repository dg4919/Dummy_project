...
namespace MyProject.Data
{
    ...
    internal static class ClassUtility
    {
        ...
        {
            return Write_Image(Base64image, image_urlResolver.school_profilePic_main);           
        }

        internal static string WriteImage(string Base64image, string path)
        {
            return Write_Image( Base64image,  path);            
        }

        private static string Write_Image(string Base64image, string path)
        {
            try
            {
                ...
                {
                    var imgPath = "";
                    if (Base64image.Contains(path))
                    {
                        int startIndex = Base64image.IndexOf(path);
                    ...
                    imgPath = path + "IMG_" + Guid.NewGuid() + "." + (imgData[0].Split('/'))[1].Split(';')[0];
        ...
        // return project path where we want to save Image > at the end of string we metioned "MyProject.Web" (project where we want to save image)
        ...
        internal static bool save_Images_toPhysical(IEnumerable<string> imageName, string tempPath, string finalPath, string fileName = null) // default + optional parameters
        {
            try
            {
                        ...
                        foreach (FileInfo file in myDirInfo.GetFiles())     // child Loop
                        {
                            if (file.Name.Equals(image))        // if image name from server is exist in temp path
                            {
                                System.IO.File.Copy
                                    ...
                                    Path.Combine(Destination, fileName ?? file.Name),
                ...
                Directory.Delete(Source, true);     // after save image to physial path remove it :slightly_smiling_face:
                return true;
            }
            catch (Exception)
            {
        ...
        public static List<string> upload_orignal_Images_toTemp(string tempPath)
        {
            var filesPath = new List<string>();
            ...
            for (var i = 0; i < files.Count; i++)
            {
                ...
                file.SaveAs(UploadFolderPath + FullfileName);
            }
            return filesPath;
        }
